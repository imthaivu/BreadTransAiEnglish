...
import { parseLearnTabParam, type LearnTabId } from "@/lib/learn-tabs";
...
function LearnPageInner({ initialTab }: { initialTab?: LearnTabId }) {
  return (
    <RequireAuth>
      <Suspense fallback={<LearnPageSpinner />}>
...
function LearnPageWithSearchParams() {
  const searchParams = useSearchParams();
  const initialTab = parseLearnTabParam(searchParams.get("tab"));
  return <LearnPageInner initialTab={initialTab} />;
}

export default function LearnPage() {
  return (
    <Suspense fallback={<LearnPageSpinner />}>
      <LearnPageWithSearchParams />
    </Suspense>
  );
}
